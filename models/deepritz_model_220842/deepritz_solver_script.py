...
class DeepRitzSolver(nn.Module):
    ...
    def __init__(self, device, sigma_h, a, fr, ft, fd, layers=[3, 64, 64, 64, 1]):
        ...
        self.bc_weight = 5.0   # Peso per le condizioni al contorno
    ...
    def get_energy_functional_loss(self, x, y, t):
        """
        Calcola la loss basata sul funzionale di energia (formulazione variazionale).
        Minimizza l'energia del sistema invece del residuo della PDE.
        """
        u = self(x, y, t)
        ...
        # Termine di energia cinetica (derivata temporale)
        kinetic_energy = 0.5 * u_t**2
        
        # Termine di energia potenziale (gradiente spaziale)
        potential_energy = 0.5 * self.sigma_h * (u_x**2 + u_y**2)
        
        # Termine di reazione (energia di interazione)
        reaction_energy = self.get_reaction_energy(u)
        
        # Funzionale di energia totale
        total_energy = kinetic_energy + potential_energy + reaction_energy
        
        return torch.mean(total_energy)

    def get_reaction_energy(self, u):
        """
        Calcola il termine di energia dovuto alla reazione nonlineare.
        Per il modello FitzHugh-Nagumo: R(u) = a*u*(u-fr)*(u-ft)*(u-fd)
        """
        # Primitiva del termine di reazione per il funzionale di energia
        # ∫ R(u) du = a * ∫ u*(u-fr)*(u-ft)*(u-fd) du
        
        # Espansione del polinomio cubico
        # u*(u-fr)*(u-ft)*(u-fd) = u^4 - u^3*(fr+ft+fd) + u^2*(fr*ft + fr*fd + ft*fd) - u*(fr*ft*fd)
        
        coeff_4 = 1.0
        coeff_3 = -(self.fr + self.ft + self.fd)
        coeff_2 = (self.fr*self.ft + self.fr*self.fd + self.ft*self.fd)
        coeff_1 = -(self.fr*self.ft*self.fd)
        
        # Primitiva: u^5/5 - u^4/4*coeff_3 + u^3/3*coeff_2 - u^2/2*coeff_1
        reaction_potential = self.a * (
            (u**5) / 5.0 +
            coeff_3 * (u**4) / 4.0 +
            coeff_2 * (u**3) / 3.0 +
            coeff_1 * (u**2) / 2.0
