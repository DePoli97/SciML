...
def train_cnn_model(model_name=None):
    ...
    history = trainer.train(
        train_loader,
        val_loader,
        num_epochs=10000,             # Triplicato il numero di epoche
        mse_weight=0.6,             # Peso della MSE loss
        mae_weight=0.4,             # Peso della MAE loss
        early_stop_patience=40      # Patience aumentata per consentire plateau nel training
    )
...
def main():
    ...
    parser.add_argument(
        'action', 
        choices=['fem', 'pinn-train', 'pinn-predict', 'cnn-train', 'cnn-predict'], 
